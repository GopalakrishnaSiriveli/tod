  ...
  return (
    <>
      ...
      <section className={styles.toDoWrapper}>
        ...
        <div className={styles.toDoListCardWrapper}>
          {(filteredToDo.length > 0 || currentTodoTask.length > 0) ? (
            ...
            <div className={styles.noTaskWrapper}>
              <Empty />
              <p>Please add any task</p>
            </div>
          )}
        </div>

        <Modal confirmLoading={loading} title="Add New To Do Task" visible={isAdding} onOk={handleSubmitTask} onCancel={() => setIsAdding(false)}>
        ...
        <Modal confirmLoading={loading} title={`Update ${currentEditItem.title}`} visible={isEditing} onOk={handleUpdateTask} onCancel={() => setIsEditing(false)}>
          <Input style={{ marginBottom: '1rem' }} placeholder='Title' value={updatedTitle} onChange={(e) => setUpdatedTitle(e.target.value)} />
          <Input.TextArea placeholder='Description' value={updatedDescription} onChange={(e) => setUpdatedDescription(e.target.value)} />
          <Select
            style={{ width: '100%', marginTop: '1rem' }}
            value={updatedStatus ? "Completed" : "Incomplete"}
            onChange={(value) => setUpdatedStatus(value === "Completed")}
            options={[
